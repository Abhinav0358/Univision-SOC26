...
# Load the reference image to simulate video frames
...
# Simulates a 10-frame video sequence by translating the image horizontally
...
for frame_num in range(10):
    ...
    shift = frame_num * 20
    # Boundary check for translation
    ...
    for box in detections:
        ...
        # Filter: only evaluate vehicle classes (COCO class 2 = car, 7 = truck)
        ...
        for track_id, prev_centroid in active_tracks.items():
            ...
            if dist < 60:  # Matching distance threshold
        ...
        # License Plate Extraction & OCR Preprocessing
        # Heuristic crop focused on lower-middle vehicle quadrant
        plate_x1 = x1 + int((x2 - x1) * 0.3)
        plate_x2 = x1 + int((x2 - x1) * 0.7)
        plate_y1 = y1 + int((y2 - y1) * 0.7)
        plate_y2 = y2

        # Clamp crop coordinates to spatial boundaries
        ...
        if plate_crop.size > 0:
            # Grayscale -> Gaussian Blur -> Binary Thresholding pipeline
        ...
        # Vehicle ID and OCR metadata label
        ...
        # Draw ROI target crop box
        cv2.rectangle(
            display_frame, (plate_x1, plate_y1), (plate_x2, plate_y2), (0, 255, 255), 2
        )

    # Persist track states for next frame iteration
